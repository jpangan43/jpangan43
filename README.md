# Jayson Pangan

<a href="https://www.linkedin.com/in/jayson-pangan-b40a8153" target="_blank">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linkedin/linkedin-original.svg" alt="LinkedIn" width="40" height="40"/>
</a>

## About Me
Aspiring SOC Analyst with a background in IT and System Administration. Passionate about cybersecurity, threat detection, and incident response. Currently building hands-on skills using security tools and home labs.

## Objective
To start a SOC Analyst career and build expertise in cybersecurity operations and threat detection.

## Skills
|        Skills         |      Associated Training     |
| --------------------- | ---------------------------- |
| System Administration | AZ-104 - Azure Administrator |


## 👨‍💻 Skills & Tools:
* **Security:** SIEM, Incident Response, Vulnerability Management
* **Tools:** Splunk, Wazuh, ELK Stack, Wireshark

## 🚀 Featured Projects:
|        Project Name      |      Associated Project      |
| ------------------------ | ---------------------------- |
| MYDFIR Elastic Challenge |    MYDFIR Elastic Challenge  |

## Certifications
<a href="https://www.credly.com/badges/07a63683-2afb-4d87-9889-5d7234621c24/public_url" target="_blank">
  <img src="https://shields.io" alt="CompTIA Security+" width="140" height="40"/></a>




